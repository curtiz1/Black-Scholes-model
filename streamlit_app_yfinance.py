...
with st.sidebar:
    ...
    #  Ticker input
    ticker = st.text_input("Enter Stock Ticker (e.g. AAPL, TSLA, INFY.BO)", value="AAPL")

    #  Try to fetch live stock price from yfinance
    current_price_live = None
    if ticker:
        try:
            stock = yf.Ticker(ticker)
            hist = stock.history(period="1d")
            if not hist.empty:
                current_price_live = hist["Close"].iloc[-1]
                st.success(f"📈 Live price of {ticker}: ${current_price_live:.2f}")
            else:
                st.warning("⚠️ Could not retrieve data for the ticker.")
        except Exception as e:
            st.error(f"❌ Error fetching price: {e}")

    # Allow manual override of price, default to live price if found
    current_price = st.number_input("Current Asset Price (override if needed)", 
                                    value=float(current_price_live) if current_price_live else 100.0)
